...
class Question extends Component {
  ...
  static async getInitialProps({ query }) {
    ...
    if(query.id) {
      id = query.id;
      poll = await axios.get(`https://torralbot-back.herokuapp.com/${id}/details`).then(res=>res.data);
      return { poll };
    }else{
      polls = await axios.get('https://torralbot-back.herokuapp.com/list').then(res=>res.data);
      return { polls };
    }
  ...
  handleFilter = (status) =>{
    ...
    render() {
      ...
      //       //const { url: { query: { id } } } = this.props;
      //       const { options, active, name }= this.props.poll;
